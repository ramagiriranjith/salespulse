...
exports.completeOrderDB = async (orderIds, tenantId) => {
  ...
  try {
      ...
      const orderIdsText = orderIds.join(",");
      const items_sql = `SELECT oi.quantity, mi.inventory_id FROM order_items oi LEFT JOIN menu_items mi ON mi.id= oi.item_id WHERE oi.order_id IN (${orderIdsText}) AND oi.tenant_id = ?`
      const [inv_items] = await conn.query(items_sql, [tenantId]);
      for await (let item of inv_items) {
          const { quantity, inventory_id } = item;

          // Update the inventory quantity by reducing the stock
          if (quantity > 0 && inventory_id) {
              await conn.query(`
                UPDATE inventory_items
                SET quantity = quantity - ?, inventory_changed_time = CURDATE()
                WHERE id = ?`, [quantity, inventory_id]);
          }
      }
...
exports.createInvoiceDB = async (subtotal, taxTotal, total, discount, date, selectedPaymentType, tenantId) => {
  const conn = await getMySqlPromiseConnection();
  try {
