...
import { getCustomerDueReport } from '../../controllers/reports.controller';
import {jsPDF} from "jspdf";
import html2canvas from "html2canvas-pro";
import toast from 'react-hot-toast';

export const CustomerDueReport = () => {
  ...
  const getReport = async () =>{
    ...
    setReportData([]);
    const data = await getCustomerDueReport(startDate, endDate);
    setReportData(data);
  }
  const totalDue = reportData.reduce((sum, item) => sum + (parseFloat(item.total_amount) || 0), 0);
  ...
  return (
    <Page>
      <>
        <h3 className="text-2xl"> Customer Due Report</h3>
          ...
          <div id="pdf-content" style={{ width: "100%", padding: "17px" }}>
            <table className='table workPeriodReport'>
              ...
              <tr>
                ...
                <td><strong>{totalDue}.00</strong></td>
              </tr>
