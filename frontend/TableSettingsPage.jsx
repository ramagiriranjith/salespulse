...
import { addNewStoreTable, deleteTable, updateStoreTable, useStoreSettings, getStoreTableDetails } from "../../controllers/settings.controller";
...
export default function TableSettingsPage() {
  ...
  const { APIURL, data: storeTables, error, isLoading } = getStoreTableDetails();
  ...
  async function btnAdd() {
    ...
    try {
      toast.loading("Please wait...");
      const res = await addNewStoreTable(title, floor, seatingCapacity, occupancyStatus);
  ...
  const btnShowUpdate = async (id, title, floor, seatingCapacity, occupancyStatus) => {
  ...
  const btnUpdate = async () => {
    ...
    try {
      toast.loading("Please wait...");
      const res = await updateStoreTable(id, title, floor, seatingCapacity, occupancyStatus);
      ...
      if (res.status == 200) {
        ...
        tableOccupancyStatusUpdateRef.current.value = null;
    ...
    <Page className="px-8 py-6">
      ...
      <div className="mt-8 w-full grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {storeTables.map((storeTable, index) => {
          const { id, table_title, seating_capacity, occupancy_status, floor, encrypted_id } = storeTable;
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-0">
                  <button
                    onClick={() => {
                      btnShowUpdate(id, table_title, floor, seating_capacity, occupancy_status);
                    }}
              ...
              }} className="btn btn-xs h-10 bg-gray-100 text-gray-500"><IconQrcode size={18} stroke={iconStroke} /> Download Table QR</button>
            </div>
          );
        })}
      </div>
      ...
      <dialog id="modal-add" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="flex-1">
            ...
            <input ref={tableTitleRef} type="text" name="title" className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Table Title" />
          </div>
            <div className="flex-1">
              <label htmlFor="floor" className="mb-1 block text-gray-500 text-sm">Floor</label>
              <input ref={tableFloorRef} type="text" name="floor" className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Floor Title" />
            </div>
          ...
          <div className="flex gap-4 w-full my-4">
            ...
            <div className="flex-1">
              ...
              <input ref={tableSeatingCapacityRef} type="number" min={0} name="capacity" className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Seating Capcity" />
            </div>
            <div className="flex-1">
              <label htmlFor="status" className="mb-1 block text-gray-500 text-sm">Occupancy Status</label>
              <select
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              <button onClick={() => { btnAdd(); }} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="flex gap-4 w-full my-4">
            <div className="flex-1 ">
              ...
              <label htmlFor="title" className="mb-1 block text-gray-500 text-sm">Title</label>
              <input ref={tableTitleUpdateRef} type="text" name="title" className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Table Title" />
            </div>
            <div className="flex-1">
              <label htmlFor="floor" className="mb-1 block text-gray-500 text-sm">Floor</label>
              <input ref={tableFloorUpdateRef} type="text" name="floor" className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Floor Title" />
            </div>
          ...
          <div className="flex gap-4 w-full my-4">
            ...
            <div className="flex-1">
              <label htmlFor="capacity" className="mb-1 block text-gray-500 text-sm">Seating Capacity</label>
              <input ref={tableSeatingCapacityUpdateRef} type="number" name="capacity" min={0} className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Seating Capcity" />
            </div>
            <div className="flex-1">
              <label htmlFor="status" className="mb-1 block text-gray-500 text-sm">Occupancy Status</label>
              <select
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={() => { btnUpdate(); }} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Save</button>
            </form>
          </div>
