...
export default function POSPage() {
  ...
  const calculateOrderSummary = () => {
    ...
    return {
      itemsTotal, taxTotal, payableTotal, total_discount
    }
  };
  const btnShowPayAndSendToKitchenModal = () => {
    ...
    const { itemsTotal, taxTotal, payableTotal, total_discount } = calculateOrderSummary();
  ...
  const btnPayAndSendToKitchen = async () => {
    try {
      ...
      toast.loading("Please wait...");
      const res = await createOrderAndInvoice(cartItems, deliveryType, customerType, customer, tableId, state.itemsTotal, state.taxTotal, state.payableTotal, state.total_discount, state.selectedQrOrderItem, state.selectedPaymentType);
      toast.dismiss();
      if(res.status == 200) {
        const data = res.data;
        toast.success(res.data.message);
      ...
      <div className='mt-4 md:h-[calc(100vh-136px)] flex flex-col-reverse md:flex-row gap-4'>
        ...
        <div className='border border-salespos-border-green-light rounded-2xl h-full md:w-[70%] overflow-y-auto'>
          ...
          {menuItems.filter((menuItem)=>{
            ...
            return <div className='bg-white border border-salespos-border-green-light rounded-2xl p-2 flex gap-2' key={i}>
        ...
        <div className='border border-salespos-border-green-light rounded-2xl h-full md:w-[30%] relative flex flex-col'>
          ...
          <div className='sticky w-full px-4 py-4 bg-gradient-to-b from-white to-white/0 border-b rounded-t-2xl'>
            ...
            <div onClick={btnOpenSearchCustomerModal} className="flex items-center gap-2">
              <input value={customerType=="WALKIN"?"WALKIN CUSTOMER":`${customer.name}`} type="text" placeholder='Search Customer' className='cursor-pointer text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light'  />
            ...
            {/* delivery type */}
            <select ref={diningOptionRef} className='mt-3 text-sm text-gray-500 w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light'>
            ...
            {/* table selection */}
            <select ref={tableRef} className='mt-3 text-sm text-gray-500 w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light'>
              <option value="">Select Table</option>
              {
                storeTables.map((table, index)=>{
                  return <option value={table.id} key={index} disabled={table.occupancy_status === 'occupied'}
                  className={table.occupancy_status === 'occupied' ? 'opacity-50 cursor-not-allowed' : ''}>
                    {table.table_title} ({table.seating_capacity} person) - {table.floor}</option>
                })
              }
            </select>
            {/* table selection */}
          ...
          <div className='flex-1 flex flex-col gap-4 overflow-y-auto px-4 pb-36'>
            ...
            {cartItems?.map((cartItem, i)=>{
              ...
              return <div key={i} className='text-sm border border-salespos-border-green-light rounded-lg p-2 relative'>
          ...
          <div className='absolute w-full bottom-0 pb-4 rounded-b-2xl px-4 bg-white/20 backdrop-blur border-t'>
            ...
            <div className="mt-2">
              <button onClick={btnShowPayAndSendToKitchenModal} className="text-sm rounded-lg border border-transparent bg-salespos-green text-white hover:bg-salespos-green-dark transition active:scale-95 hover:shadow-lg px-2 py-1 w-full flex items-center justify-center gap-1">
      ...
      <dialog id="modal-notes" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4">
            ...
            <input ref={dialogNotesTextRef} type="text" name="dialogNotesText" id='dialogNotesText' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Notes here..." />
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              <button onClick={()=>{btnAddNotes();}} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={()=>{btnAddMenuItemToCartWithVariantsAndAddon()}} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      <dialog id="modal-save-draft" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4">
            ...
            <input ref={draftTitleRef} type="text" name="draftTitleRef" id='draftTitleRef' className="text-sm w-full border rounded-lg px-4 py-2 bg-gray-50 outline-salespos-border-green-light" placeholder="Enter Reference Name here..." />
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={()=>{btnAddtoDrafts();}} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Save</button>
            </form>
          </div>
        </div>
      </dialog>
            ...
            {drafts.map((draftItem, index)=>{
              ...
              return <div key={index} className='flex items-center gap-1 rounded-2xl p-2 border'>
                ...
                <div className="flex flex-col gap-1">
                  <button onClick={()=>{btnSelectDraftItemToCart(draftItem)}}  className="rounded-full transition active:scale-95 text-salespos-green w-6 h-6 bg-green-50 hover:bg-salespos-green-dark hover:text-white flex items-center justify-center"><IconPencil size={14} stroke={iconStroke} /></button>
      ...
      <dialog id="modal-send-kitchen-summary" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="my-4 flex items-center divide-x w-full">
            ...
            <div className="flex-1 pl-4">
              <p>Payable Total</p>
              <p className="text-lg font-bold text-salespos-green">{currency}{state.payableTotal.toFixed(2)}</p>
            </div>
            <div className="flex-1 text-center">
              <p>Discount Total</p>
              <p className="text-lg font-bold text-salespos-green">{currency}{state.total_discount.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="modal-action">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={btnSendToKitchen} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Send to Kitchen</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="modal-action flex items-center justify-center w-full">
            <form method="dialog" className='w-full'>
              {/* if there is a button in form, it will close the modal */}
              <button onClick={()=>{btnPayAndSendToKitchen();}} className="w-full text-center rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white">Collect Payment & Send to Kitchen</button>
            </form>
          </div>
        </div>
      </dialog>
        ...
        <div className="modal-box">
          ...
          <div className="modal-action justify-center">
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Close</button>
              <button onClick={btnPrintTokenOnly} className="rounded-lg hover:bg-red-400 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-salespos-green text-white ml-3">Print Token</button>
            </form>
          </div>
        </div>
      </dialog>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 pb-6">
            {state?.qrOrders?.map((qrOrder, index)=>{
              ...
              return <div key={index} className='flex items-center gap-1 rounded-2xl p-2 border'>
                ...
                <div className="flex flex-col gap-1">
                  <button onClick={()=>{btnSelectQROrder(qrOrder)}}  className="rounded-full transition active:scale-95 text-salespos-green w-6 h-6 bg-green-50 hover:bg-salespos-green-dark hover:text-white flex items-center justify-center"><IconPencil size={14} stroke={iconStroke} /></button>
